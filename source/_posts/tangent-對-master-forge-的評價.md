...
但在 M4G 上，拇指以外的搖桿的方向與我對應的手指的方向的差異很大。這導致 M4G 使用起來相對較不直覺。並且如果我用在 CC1 和 CC2 上的肌肉記憶來使用 M4G 時，會很容易按錯按鍵。舉例來說，當我想按左手的食指南鍵時，我可能會反而誤按到西鍵。

（[這個問題的詳細說明我寫在附錄 A。](#A-關於搖桿方向問題的詳細說明)）
...
在一些秀出 M4G 的電路板的影片中，例如[這個影片](https://youtu.be/gu8rpQNdFg4?feature=shared)，可以看到拇指以外的搖桿的方向是與 Digitizer（半個 M4G）對齊的。如果我把四個方向用紅色十字符號標在先前的照片上，會看起來像這樣。

![image (1)](https://hackmd.io/_uploads/r1T6-Uhb-x.png)

你可以看到我的拇指以外的手指和搖桿方向間大約有 45 度的角度，因此我需要斜向移動我的手指來按到正確的鍵。比如需要將左食指往東北方移來按 `r` 鍵、往東南方移來按到 `e` 鍵。這對我來說相當不直覺。此外，你可以想像我使用方向鍵或滑鼠移動鍵時有多麼不直覺。
